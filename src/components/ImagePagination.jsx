...
function ImagePagination({length, current, setCurrent}) {
    return (
        <div className={"flex h-fit m-2 justify-center items-center"}>
            {
                length > 1 && <>
                    ...
                    </button>
                    {
                        Array(length).fill(0).map((i, j) => {
                            if(j === current) {
                                return <span className={`line ${j === length - 1 ? '' : 'mr-1'}`} />
                            } else
                                return <span className={`dot ${j === length - 1 ? '' : 'mr-1'}`} />
                        })
                    }
