        ...
        <div className={"fixed top-0 left-0 h-full lg:relative flex flex-col overflow-scroll lg:m-2 lg:rounded-xl lg:ml-0 w-full lg:w-1/4 "}>
            ...
            <div className={"bg-white"}>
                {permission &&
                    <QrReader className={"lg:rounded-xl scanner"} onScan={onScan} onError={(err) => console.log(err)}
                              delay={500} facingMode={"environment"} showViewFinder={true}/>}
            </div>
            <div className={"rounded-t-2xl flex flex-col justify-center items-center flex-1 z-10 p-4 -mt-10 border-t bg-white"}>
                <div className={"m-auto flex flex-col items-center pb-10"}>
                    ...
                    {
                        permission ? <span className={"font-Poppins font-[500] my-4"}>Scan QR code to know more!</span>
                            :
                            <span className={"font-Poppins font-[400] my-4"}>Requires permission to access camera</span>
                    }
                </div>
            </div>
        </div>
